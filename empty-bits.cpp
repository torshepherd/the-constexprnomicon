// Clang 22.1.0, -std=c++23. Constant-evaluation experiment, not runtime storage.
...
template<unsigned... I> struct bits : bit<I>... {
    constexpr bits(unsigned long long n = 0) { set(n); }
    // Revive the bases before implicit base destruction.
    constexpr ~bits() { (std::construct_at(static_cast<bit<I>*>(this)), ...); }
    constexpr unsigned long long get() const {
        return ((1ull * __builtin_constant_p(static_cast<const bit<I>*>(this)->alive()) << I) | ...);
    }
    constexpr void set(unsigned long long n) {
        auto set_bit = [&]<unsigned J>() {
