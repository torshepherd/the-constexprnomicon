...
| Object layout under the checked Itanium-style ABI | [Chromatic aberration](chromatic-aberration.cpp) computes first-fit graph coloring. The same offset rule composes NAND gates; [all four XOR cases pass](working-notes/layout-xor.cpp). Intermediate signals remain in object offsets. | The circuit's gates are explicitly declared. No way to repeat transitions beyond those components, without adding another mechanism, has been demonstrated. |
